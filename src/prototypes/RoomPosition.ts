...
Object.defineProperty(RoomPosition, 'name', {
	get: function(this: RoomPosition): string {
		return `${this.roomName}:${this.x}:${this.y}`;
	},
	configurable: true,
});

Object.defineProperty(RoomPosition, 'coordinateName', {
	get: function(this: RoomPosition): string {
...
Object.defineProperty(RoomPosition, 'room', {
	get: function(this: RoomPosition): Room {
...
RoomPosition.prototype.capacity = function(this: RoomPosition, range: number = 1): number {
	...
	let maxPositions =
		abs(min(x + range, 49) - max(x - range, 0) + 1) * abs(min(y + range, 49) - max(y - range, 0) + 1) - 1;
